...
#define DANOS_FAT16_FAT_ENTRY_SIZE 0x02
#define DANOS_FAT16_BAD_SECTOR 0xFF7
#define DANOS_FAT16_UNUSED 0x00
...
struct fat_private
{
    ...
    // Used to strema the file allocation table
...
int fat16_get_total_items_for_directory(struct disk* disk, uint32_t directory_start_sector)
{
    ...
    if (diskstreamer_seek(stream, directory_start_pos) != DANOS_ALL_OK)
    {
        res = -DANOS_EIO;
        goto out;
    }
    ...
    while(1)
    {
        if (diskstreamer_read(stream, &item, sizeof(item)) != DANOS_ALL_OK)
        {
            res = -DANOS_EIO;
            goto out;
        }
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* fat_private, struct fat_directory* directory)
{
    ...
    struct disk_stream* stream = fat_private->directory_stream;
    if (diskstreamer_seek(stream, fat16_sector_to_absolute(disk, root_dir_sector_pos)) != DANOS_ALL_OK)
    {
        res = -DANOS_EIO;
        goto err_out;
    }

    if (diskstreamer_read(stream, dir, root_dir_size) != DANOS_ALL_OK)
    {
        res = DANOS_EIO;
        goto err_out;
    }
...
int fat16_resolve(struct disk* disk)
{
    ...
    if (diskstreamer_read(stream, &fat_private->header, sizeof(fat_private->header)) != DANOS_ALL_OK)
    {
        res = -DANOS_EIO;
        goto out;
    }

    if (fat_private->header.shared.extended_header.signature != 0x29) // number in the fat manual
    {
        res = -DANOS_EFSNOTUS;
        goto out;
    }

    if (fat16_get_root_directory(disk, fat_private, &fat_private->root_directory) != DANOS_ALL_OK)
    {
        res = -DANOS_EIO;
        goto out;
    }
...
void fat16_get_full_relative_filename(struct fat_directory_item* item, char* out, int max_len)
{
    ...
    char* out_tmp = out;
    fat16_to_proper_string(&out_tmp, (const char*) item->filename, sizeof(item->filename)); // Filename is padded to fill up 8 bytes! this shortens is

    if (item->ext[0] != 0x00 && item->ext[0] != 0x20)
    {
        *out_tmp++ = '.';
        fat16_to_proper_string(&out_tmp, (const char*) item->ext, sizeof(item->ext)); // extension is padded to fill up 3 bytes!
    }
}
...
{
    return private->header.primary_header.reserved_sectors; // first FAT comes after reserved sectors
}
...
static int fat16_get_fat_entry(struct disk* disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * DANOS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
// Get the correct cluster fo use based on the strating cluster and the offset
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        // Sector is makred as bad?
        if (entry == DANOS_FAT16_BAD_SECTOR)
        {
            res = -DANOS_EIO;
            goto out;
        }
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name)
{
    ...
    char tmp_filename[DANOS_MAX_PATH];
