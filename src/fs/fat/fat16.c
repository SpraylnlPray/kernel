...
void fat16_get_full_relative_filename(struct fat_directory_item* item, char* out, int max_len)
{
    ...
    char* out_tmp = out;
    fat16_to_proper_string(&out_tmp, (const char*) item->filename); // Filename is padded to fill up 8 bytes! this shortens is

    if (item->ext[0] != 0x00 && item->ext[0] != 0x20)
    {
        *out_tmp++ = '.';
        fat16_to_proper_string(&out_tmp, (const char*) item->ext); // extension is padded to fill up 3 bytes!
    }
}
