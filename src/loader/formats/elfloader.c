...
{
    return header->e_type == ET_EXEC && header->e_entry >= DANOS_PROGRAM_VIRTUAL_ADDRESS;
}
...
{
    return &elf_header(header)[index];
}
...
{
    return (elf_valid_signature(header) && elf_valid_class(header) && elf_valid_encoding(header) && elf_has_program_header(header)) ? DANOS_ALL_OK : -DANOS_EINVARG;
}
